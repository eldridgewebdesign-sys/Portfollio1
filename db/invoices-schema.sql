...
-- Invoices are one-time only. If this table was created by an earlier migration
-- that added recurring-billing columns, drop them — they are redundant now (see
-- db/invoices-one-time-cleanup.sql for the standalone teardown). Idempotent.
...
-- A previous migration created public.invoice_payments to track RECURRING invoice
-- renewals. Invoices are one-time only now, so that table is redundant — drop it
-- (idempotent; the standalone teardown lives in db/invoices-one-time-cleanup.sql).
...
-- ---------------------------------------------------------------------
-- Drop any earlier overloads so the one-time signature below is the ONLY one —
-- otherwise PostgREST could see two and fail with "not unique". Covers both the
-- original (pre-billing_type) and the billing_type variants. Safe to re-run.
